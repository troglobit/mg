 ...
 * │ Display width uses a built-in Unicode table.                │
 * │                                                             │
...
/*
 * Adapted from Markus Kuhn's wcwidth.c, 2007-05-26 (Unicode 5.0).
...
{
	/* Sorted, non-overlapping intervals of non-spacing characters. */
...
/* Number of display columns; combining characters still need their own cell. */
int
utf8_width(int cp)
{
	int width;

	width = mk_wcwidth((uint32_t)cp);
