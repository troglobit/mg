...
Meta (usually labelled "Alt") key. We will use the following conventions in
this tutorial:
...
This tutorial is your own private copy: type away and experiment as much as
you like. Nothing is saved unless you ask for it.
...
Finally, to jump to a specific line, type M-g g followed by the line number
and Enter. The older C-x g binding does the same thing.
...
M-backspace (Meta and backspace key), you will delete one word instead of
...
To stop searching for text, use the cursor keys (or C-f, C-b etc.) or C-g
to stop the search operation.
...
When you set the mark with C-space, mg highlights the region between the
mark and your cursor in reverse video, so you can see exactly what C-w or M-w
will act on. You can make the same selection by holding Shift while moving:
Shift with an arrow key grows the region as you go, adding Control extends by
whole words, and the first unshifted key drops the selection again.

> Hold Shift and press the right arrow key a few times, and watch the region
> grow. Press a plain arrow key to let it go.
...
you're reading the buffer holding this tutorial. If you've edited this file and not
...
In the middle of the status line, the current editing modes are shown.  Every
buffer has the basic "fundamental" mode; on top of that, mg has language modes
for C, shell scripts, Python and Markdown, which color the text and adjust
indentation to fit the language. They turn on by themselves when a file's name
or contents give the language away.  See the man page for mg(1) to learn more
about the editing modes.
...
If you decide to open a directory instead of a file, mg will transition into a
...
buffer. For example, with the cursor over a specific file:
    ...
    Pressing c      - gives you the opportunity to copy the file
    Pressing d      - marks the file for deletion
    Pressing x      - unlink files previously marked for deletion
    Pressing Return - open the file into it's own buffer for editing
...
The screen can also be split side by side with C-x 3, and both kinds of split
can be mixed freely. With several windows on screen, the quickest way to move
between them is Meta and an arrow key, which selects the window in that
direction. Meta, Shift and an arrow key moves the window edge the same way,
making windows larger or smaller, and C-x + evens all the sizes out again.

> Use C-x 3 to split the screen side by side, and Meta with the left and
> right arrow keys to move between the windows.

> Hold Meta and Shift and press the left arrow key a few times to move the
> divider, type C-x + to even the windows out, and C-x 1 when done.
